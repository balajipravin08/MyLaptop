...
public enum Queries {
	ADD_SUPER_ADMIN("insert into employee(name,mobile,location,email,password,job,salary,specialist) values ('Balaji','7540086115','SRIVILLIPUTHUR','balaji@gmail.com','Balaji@123',2,'0','') ON CONFLICT (email) DO NOTHING"),
	...
	CREATE_PATIENT("CREATE TABLE IF NOT EXISTS patient(id BIGSERIAL NOT NULL PRIMARY KEY,name VARCHAR(15) NOT NULL,mobile VARCHAR(10) NOT NULL UNIQUE,location VARCHAR(50) NOT NULL, email VARCHAR(50) NOT NULL UNIQUE,password VARCHAR(11) NOT NULL,bloodgroup VARCHAR(5) NOT NULL,status BOOLEAN DEFAULT TRUE)"),
	CREATE_EMPLOYEE("CREATE TABLE IF NOT EXISTS employee(id BIGSERIAL NOT NULL PRIMARY KEY,name VARCHAR(15) NOT NULL,mobile VARCHAR(10) NOT NULL UNIQUE,location VARCHAR(50) NOT NULL, email VARCHAR(50) NOT NULL UNIQUE,password VARCHAR(11) NOT NULL,job SMALLINT NOT NULL,salary BIGSERIAL NOT NULL,specialist VARCHAR(20) NOT NULL,status BOOLEAN DEFAULT TRUE)"),
	...
	VIEW_ALL_APPOINTMENT("select appointment.id,patient.name,employee.name,date,appointment.status from appointment join patient on patient.id=appointment.patient_id join employee on employee.id=appointment.doctor_id where date = (SELECT CURRENT_DATE) and appointment.status = true "),
